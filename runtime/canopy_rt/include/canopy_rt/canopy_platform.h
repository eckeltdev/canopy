#ifndef CANOPY_PLATFORM_H
#define CANOPY_PLATFORM_H

/*
 * canopy_platform.h — the Canopy platform seam (the C ABI FLOOR).
 *
 ...
 *   The (size, align) passed to canopy_platform_free are ADVISORY HINTS, never required.
...
#define CANOPY_NORETURN [[noreturn]]
extern "C" {
#else
#define CANOPY_NORETURN _Noreturn
#endif
...
void *canopy_platform_alloc(size_t size, size_t align);

/* Free a block from canopy_platform_alloc. `size`/`align` are advisory (see FREE CONTRACT).
 * Freeing NULL is a no-op. */
void canopy_platform_free(void *ptr, size_t size, size_t align);

/* Abort the program with a diagnostic message; never returns. Called on OOM and on
 * unrecoverable invariant violations. */
CANOPY_NORETURN void canopy_platform_panic(const char *msg);

/* Write `len` bytes of log text (not necessarily NUL-terminated). */
void canopy_platform_log(const char *msg, size_t len);

/* A monotonic tick counter and its frequency (ticks per second), for timing. */
uint64_t canopy_platform_ticks(void);
uint64_t canopy_platform_ticks_per_second(void);

#if defined(__cplusplus)
} /* extern "C" */
#endif

#endif /* CANOPY_PLATFORM_H */
