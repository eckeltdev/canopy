...
#include "canopy_rt/debug_backend.hpp"

// The headline M1 proof: with the canopy-rt global operator new/delete linked in, plain std
...
namespace {
    ...
    bool std_containers_route_through_the_seam() {
        canopy::rt::install_debug_backend();
        canopy::rt::debug_backend_reset();

        const canopy::rt::debug_stats start = canopy::rt::debug_backend_stats();
        {
        ...
        const canopy::rt::debug_stats finish = canopy::rt::debug_backend_stats();
...
int main() {
    if (std_containers_route_through_the_seam()) {
        std::cerr << "ok: std containers route through canopy-rt (alloc==free, hints matched)\n";
        return 0;
    }
    return 1;
}
