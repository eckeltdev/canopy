...
#include "canopy_rt/containers.hpp"
#include "canopy_rt/stl.hpp"

// rt_surface_smoke.cpp — M2 STL-surface smoke test.
//
// Built with -fno-exceptions -fno-rtti -DCANOPY_RT_FREESTANDING=1 (see cpp-doctor.toml), so
...
namespace {
    ...
    auto fixed_vector_overflow_returns_false() -> bool {
        canopy::rt::fixed_vector<int, 4> values;
    ...
    auto arena_bumps_and_resets() -> bool {
        alignas(std::max_align_t) std::array<canopy::rt::byte, 128> backing{};
        canopy::rt::arena scratch{canopy::rt::span<canopy::rt::byte>{backing}};
    ...
    auto aliases_are_usable() -> bool {
        constexpr canopy::rt::array<int, 3> source{10, 20, 30};
        const canopy::rt::span<const int> view{source};
        const canopy::rt::pair<int, int> ends{view.front(), view.back()};
        const canopy::rt::optional<int> present{ends.second};
        const canopy::rt::string_view label{"canopy"};

        const canopy::rt::expected<int, int> ok =
            present.has_value() ? canopy::rt::expected<int, int>{present.value()}
                                : canopy::rt::expected<int, int>{canopy::rt::unexpected<int>{-1}};

        auto owned = canopy::rt::unique_ptr<int>{std::make_unique<int>(ends.first)};

        const bool good = view.size() == source.size() && ends.first == 10 && ends.second == 30 &&
                          ok.has_value() && ok.value() == 30 && *owned == 10 && label.size() == 6 &&
                          sizeof(canopy::rt::byte) == 1;
        if (!good) {
...
int main() {
    const bool all_passed = fixed_vector_overflow_returns_false() && arena_bumps_and_resets() &&
                            aliases_are_usable();
    if (all_passed) {
        std::fprintf(stderr, "ok: canopy-rt STL surface smoke passed\n");
        return 0;
    }
    return 1;
}
