...
namespace {
    ...
    auto probe_alloc(std::size_t size, std::size_t align) -> void* {
        probe_calls() += 1;
        return canopy::rt::host_ops().alloc(size, align);
    }
    void probe_free(void* ptr, std::size_t size, std::size_t align) {
        canopy::rt::host_ops().free(ptr, size, align);
    }
    void probe_panic(const char* msg) {
        canopy::rt::host_ops().panic(msg);
    }
    void probe_log(const char* msg, std::size_t len) {
        canopy::rt::host_ops().log(msg, len);
    }
    auto probe_ticks() -> std::uint64_t {
        return canopy::rt::host_ops().ticks();
    }
    auto probe_ticks_per_second() -> std::uint64_t {
        return canopy::rt::host_ops().ticks_per_second();
    }
    constexpr canopy::rt::platform_ops probe_ops_table = {
        .alloc = probe_alloc,
        .free = probe_free,
        .panic = probe_panic,
        .log = probe_log,
    ...
    bool default_backend_round_trips() {
        void* ptr = canopy_platform_alloc(64, 16);
        if (ptr == nullptr) {
            std::cerr << "FAIL: default alloc returned null\n";
            return false;
        }
        canopy_platform_free(ptr, 64, 16);
        return true;
    }

    bool ticks_are_monotonic_with_a_frequency() {
        const std::uint64_t start = canopy_platform_ticks();
        const std::uint64_t later = canopy_platform_ticks();
        if (later < start) {
            std::cerr << "FAIL: ticks went backwards\n";
            return false;
        }
        if (canopy_platform_ticks_per_second() == 0) {
    ...
    bool installing_a_backend_reroutes_the_seam() {
        canopy::rt::install_platform(probe_ops_table);
        const int before = probe_calls();
        void* ptr = canopy_platform_alloc(32, 8);
        const int after = probe_calls();
        canopy_platform_free(ptr, 32, 8);
        if (after != before + 1) {
