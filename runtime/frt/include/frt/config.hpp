...
// config.hpp — the frt freestanding POLICY header.
//
...
// FRT_FREESTANDING when you build the freestanding-safe configuration (the device, and
...
//
// FRT_PANIC(msg) is the one escape hatch: it routes to frt_platform_panic (the C ABI
...
#if defined(FRT_FREESTANDING)

#if defined(__cpp_exceptions)
static_assert(false, "frt freestanding build requires -fno-exceptions "
                     "(__cpp_exceptions must be undefined)");
#endif

#if defined(__cpp_rtti)
static_assert(false, "frt freestanding build requires -fno-rtti "
                     "(__cpp_rtti must be undefined)");
#endif

#endif // FRT_FREESTANDING
