...
// containers.hpp — heap-free inline containers for the frt hot path.
//
...
namespace frt {
    ...
    public:
        ...
        // debug build the FRT_PANIC in back() catches the empty case.
        ...
        [[nodiscard]] auto back() -> ElementType& {
            if (size_ == 0) {
                FRT_PANIC("fixed_vector::back() on empty container");
            }
