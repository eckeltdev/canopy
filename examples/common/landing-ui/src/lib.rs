//! Shared UI for the Canopy **landing** examples — a stunning one-page (no-scroll)
//! welcome screen, authored once here and rendered two ways.
//!
//! `examples/lite/landing` rasterizes this tree on the **CPU** (the software
//! `canopy-render-text` path); `examples/full/landing` rasterizes the *same* tree on
//! the **GPU** (`canopy-render-vello`, Metal/Vulkan). Keeping the UI + animation
//! timeline in one crate makes that the whole point: identical app, swap the backend —
//! and the two can't drift. Each tier crate owns only its two binaries (a headless
//! still → PPM and the animated window).
//!
//! Dark, minimal, and animated, in the spirit of a modern product landing (design
//! inspiration: x.ai). It shows off what Canopy can do now that the core has opacity +
...
/// It lives next to this shared crate, so a single edit hot-reloads **both** the CPU and
/// GPU landing windows.
