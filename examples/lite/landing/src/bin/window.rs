//! The windowed Canopy **landing**, CPU-rasterized (macOS / Windows / Linux) — the lite
//! path.
//!
...
//!
//! Each frame is software-rasterized by [`canopy_render_text::render_dom_with`] (the CPU
//! path); the GPU twin (`examples/full/landing`) swaps in `canopy-render-vello` here and
//! is otherwise identical.
//!
//! It also **hot-reloads `styles.css`**: edit the shared stylesheet (it lives in
//! `canopy-landing-ui`, so the edit reloads both the CPU and GPU windows), save, and the
//! live window restyles.
//!
//! Run from `examples/lite/landing`: `cargo run`.
