//! The windowed Canopy landing, **GPU-rasterized** (macOS / Windows / Linux).
//!
//! This is the GPU twin of `canopy-landing`'s window: same winit window, same
//! `canopy_anim::Timeline` ticked each frame at ~60 fps via `ControlFlow::WaitUntil`,
//! same hot-reload-on-keypress. The *only* difference is the per-frame pipeline:
//!
...
//!
//! Run from `examples/canopy-landing-gpu`: `cargo run`.

use std::num::NonZeroU32;
use std::rc::Rc;
use std::time::{Duration, Instant};

use canopy_landing_gpu::{build, load_styles, Landing, VIEW_H, VIEW_W};
