//! Headless **GPU** renderer for the Canopy landing: builds the page, settles the
//! entrance animation, lays it out into a display list, and rasterizes one frame on
//! the GPU (Metal on this Mac) to a PPM. No window — this proves the *GPU* path renders
//! the exact same landing the CPU `canopy-landing` example does.
//!
...
use canopy_landing_gpu::{build, VIEW_H, VIEW_W};
