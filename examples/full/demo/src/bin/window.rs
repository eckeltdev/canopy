...
//! [`canopy_full_demo::build`]; this file is only the platform glue, which is exactly the
...
//!   layout ([`canopy_full_demo::hover_target`]); when the hovered button changes, the old
...
//!
//! Run from `examples/full/demo`: `cargo run` (CPU sharp text), or
...
use canopy_dom::Dom;
use canopy_full_demo::{build, click_handler, hover_target, run_plugin, Demo, VIEW_H, VIEW_W};
